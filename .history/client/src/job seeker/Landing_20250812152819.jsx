...
const Landing = () => {
  const { getAllJob, jobData, loading } = useContext(Context);
  useEffect(() => {
    getAllJob();
  }, []);
  return (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {jobData.map((job) => (
            ...
            >
              ...
              <p className="text-sm mt-2 text-gray-200">{job.requirements}</p>
              <p className="text-sm mt-2 text-gray-200">{job.Skills}</p>
              <p className="text-sm mt-2 text-gray-200">{job.deadline}</p>
